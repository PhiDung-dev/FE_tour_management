...
export default function PaymentManagementPage() {
  ...
  const normalizedPayments = useMemo(() => {
    return payments.map((payment) => {
      ...
      const tour = booking.tour || schedule.tour || {};
      ...
      return {
        ...
        tourTitle: tour.title || "Chưa cập nhật",
  ...
  const stats = useMemo(() => {
    return normalizedPayments.reduce(
      (result, payment) => {
        if (payment.status === "PAID") result.paid += 1;
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-500">
                Danh sách booking đã duyệt. Nhân viên kiểm tra chuyển khoản và
                cập nhật trạng thái thanh toán cho khách.
              </p>
            </div>

            <div className="inline-flex w-max items-center gap-2 rounded-md bg-blue-50 px-4 py-2 font-semibold text-blue-600">
              <WalletCards size={18} />
              Payment
            </div>
          </div>
              ...
              <tbody>
                {filteredPayments.map((payment) => (
                  ...
                  >
                    ...
                    <td className="p-3 text-slate-600">{payment.tourTitle}</td>
